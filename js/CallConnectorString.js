/* CS Call Connector String  */
jQuery(function ($) {
    $(".btnFill").on('click',function (event) {
        var strtst = $("#Ipsum").val() + $("#Ipsum").val();
        strtst = strtst.substr(0,3900);
        $(".ContentSt").val('len:'+strtst.length+':'+strtst);
        return false;
    }),
    ...
    $(".btnNext").on('click', function (event) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (data.Cd == 0) {
                    $.ajax({
                        url: ServiceUrl+"api/v1/DataCCtor/" + $('#Id').val(),
                        type: 'PUT',
                        dataType: "json",
                        contentType: 'application/json',
    ...
    $(".btnRead").on("click", function (event) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                if (data.Cd == 0) {
                    $.ajax({
                        url: ServiceUrl+"api/v1/DataCCtor/" + $('#Id').val() + '?Ix=' + $('#Idx').val() + '&Ky=' + encodeURIComponent(k2),
                ...
                if (data.Cd == 0) {
                    ...
                    for (var i = 0; i < cnt; i++) {
                        $.ajax({
                            ...
                            success: function (data, textStatus, jqXHR) {
                                if (data.Cd == 0) {
                                    cntvals = cntvals + '-i='+i+' Read len=' + data.St.length;
                                }
                                else {
                                    errVal = "Status Ok but Error: " + data.Cd;
                                    cntvals = cntvals + "[" + "Status Ok but Error: " + data.Cd + "]";
